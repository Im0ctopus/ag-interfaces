...
<div class="flex flex-col just-center items-center gap-2 w-full">
  {#each showAgents as { id, name }, index (index)}
    <button
      onclick={() => onSelect(id)}
      class="w-full rounded px-3 py-2 text-start text-sm font-semibold transition-all truncate {selected ===
      id
        ? 'bg-neutral-800 text-white dark:text-neutral-900 dark:bg-white'
        : 'cursor-pointer bg-white dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-600 dark:text-neutral-400 active:scale-97'}"
    >
      {name}
    </button>
  {/each}
